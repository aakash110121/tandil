...
type RouteParams = { offer: AdminExclusiveOffer };

const AdminEditExclusiveOfferScreen: React.FC = () => {
  const { t } = useTranslation();
  const navigation = useNavigation<any>();
  const route = useRoute<{ params: RouteParams }>();
  const initialOffer = route.params?.offer;
  const offerId = initialOffer?.id;
  ...
  const applyOfferToForm = useCallback((offer: AdminExclusiveOffer) => {
    setTitle(offer.title ?? offer.name ?? '');
    setDescription(offer.description ?? '');
    const dt = (offer.discount_type ?? 'percentage') as (typeof DISCOUNT_TYPES)[number];
    setDiscountType(DISCOUNT_TYPES.includes(dt) ? dt : 'percentage');
    setDiscountValue(offer.discount_value != null ? String(offer.discount_value) : '');
    setAppliesTo(offer.applies_to ?? '');
    setStartDate(offer.start_date ?? offer.valid_from ?? '');
    setEndDate(offer.end_date ?? offer.valid_to ?? '');
    setIsActive(offer.is_active === true || offer.is_active === 1);
    setSortOrder(offer.sort_order != null ? String(offer.sort_order) : offer.priority != null ? String(offer.priority) : '0');
    const pids = offer.product_ids;
    if (Array.isArray(pids)) {
      setSelectedProductIds(pids.filter((id): id is number => typeof id === 'number'));
    } else if (typeof pids === 'string' && pids.trim()) {
      setSelectedProductIds(
        (pids as string).split(',').map((s) => parseInt(s.trim(), 10)).filter((n) => !Number.isNaN(n))
      );
    } else {
      setSelectedProductIds([]);
    }
  ...
  useEffect(() => {
    if (initialOffer) applyOfferToForm(initialOffer);
  }, [initialOffer, applyOfferToForm]);

  const fetchOfferById = useCallback(async () => {
    if (!offerId) return;
    setLoadingOffer(true);
    try {
      const res = await adminService.getExclusiveOfferById(offerId);
      if (res.data) applyOfferToForm(res.data);
    } catch (_) {
      // keep form from initialOffer
    } finally {
      setLoadingOffer(false);
    }
  }, [offerId, applyOfferToForm]);
  ...
  const handleUpdate = async () => {
    ...
    setLoading(true);
    try {
      await adminService.updateExclusiveOffer(offerId, {
        title: title.trim(),
      ...
      Alert.alert(
        ...
        t('admin.exclusiveOffers.updateSuccess', 'Offer updated successfully.'),
      ...
      const message =
        ...
        err.message ||
        t('admin.exclusiveOffers.updateFailed', 'Failed to update offer. Please try again.');
  ...
  if (!offerId) {
    return (
      <SafeAreaView style={styles.container} edges={['bottom']}>
        <Header title={t('admin.exclusiveOffers.editOffer', 'Edit offer')} showBack onBackPress={() => navigation.goBack()} />
        <View style={styles.centerWrap}>
          <Text style={styles.errorText}>{t('admin.exclusiveOffers.offerNotFound', 'Offer not found')}</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.container} edges={['bottom']}>
      <Header
        title={t('admin.exclusiveOffers.editOffer', 'Edit Exclusive Offer')}
        showBack={true}
      ...
      >
        ...
        >
          ...
          <Input label={t('admin.exclusiveOffers.titleLabel', 'Title')} placeholder="e.g. Farm Fresh Week" value={title} onChangeText={(v) => { setTitle(v); if (errors.title) setErrors({ ...errors, title: '' }); }} error={errors.title} />
          <Input label={t('admin.exclusiveOffers.descriptionLabel', 'Description')} placeholder="e.g. Up to 30% OFF" value={description} onChangeText={setDescription} multiline />
          <View style={styles.fieldRow}>
            <Text style={styles.label}>{t('admin.exclusiveOffers.discountTypeLabel', 'Discount type')}</Text>
            <TouchableOpacity style={[styles.dropdown, errors.discount_type && styles.dropdownError]} onPress={() => setShowDiscountTypeModal(true)}>
          ...
          <Input label={t('admin.exclusiveOffers.discountValueLabel', 'Discount value')} placeholder="e.g. 30" value={discountValue} onChangeText={setDiscountValue} keyboardType="decimal-pad" />
          <Input label={t('admin.exclusiveOffers.appliesToLabel', 'Applies to')} placeholder="e.g. fresh produce" value={appliesTo} onChangeText={setAppliesTo} />
          <View style={styles.rowTwo}>
            <View style={styles.halfField}>
              <Input label={t('admin.exclusiveOffers.startDateLabel', 'Start date')} placeholder="YYYY-MM-DD" value={startDate} onChangeText={setStartDate} />
            </View>
            <View style={styles.halfField}>
              <Input label={t('admin.exclusiveOffers.endDateLabel', 'End date')} placeholder="YYYY-MM-DD" value={endDate} onChangeText={setEndDate} />
            </View>
          </View>
          <View style={styles.switchRow}>
            <Text style={styles.label}>{t('admin.exclusiveOffers.isActiveLabel', 'Active')}</Text>
            <Switch value={isActive} onValueChange={setIsActive} trackColor={{ false: COLORS.border, true: COLORS.primary + '99' }} thumbColor={isActive ? COLORS.primary : COLORS.textSecondary} />
          </View>
          <Input label={t('admin.exclusiveOffers.sortOrderLabel', 'Sort order')} placeholder="0" value={sortOrder} onChangeText={setSortOrder} keyboardType="number-pad" />
          <View style={styles.fieldRow}>
            ...
            <TouchableOpacity style={styles.productSelectBtn} onPress={() => setShowProductModal(true)}>
              <Text style={styles.productSelectBtnText} numberOfLines={1}>
                {productsLoading ? t('admin.exclusiveOffers.loadingProducts', 'Loading…') : selectedProductIds.length > 0 ? t('admin.exclusiveOffers.selectedCount', { count: selectedProductIds.length, defaultValue: '{{count}} selected' }) : t('admin.exclusiveOffers.selectProducts', 'Select products')}
              </Text>
              <Ionicons name="chevron-forward" size={20} color={COLORS.textSecondary} />
            </TouchableOpacity>
            {selectedProductIds.length > 0 && selectedProductNames ? <Text style={styles.selectedNames} numberOfLines={2}>{selectedProductNames}</Text> : null}
          </View>
          <View style={styles.fieldRow}>
            <Text style={styles.label}>{t('admin.exclusiveOffers.imageLabel', 'Image')}</Text>
            <TouchableOpacity style={[styles.imageBtn, pickingImage && styles.imageBtnDisabled]} onPress={pickImageFromDevice} disabled={pickingImage}>
          ...
          <View style={styles.modalBox} onStartShouldSetResponder={() => true}>
            ...
            {DISCOUNT_TYPES.map((type) => (
              <TouchableOpacity
                ...
                onPress={() => { setDiscountType(type); setShowDiscountTypeModal(false); if (errors.discount_type) setErrors({ ...errors, discount_type: '' }); }}
              >
          ...
          {productsLoading ? (
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                ...
                return (
                  <TouchableOpacity style={styles.productRow} onPress={() => toggleProduct(item.id)} activeOpacity={0.7}>
              ...
              ListEmptyComponent={<View style={styles.modalEmpty}><Text style={styles.modalEmptyText}>{t('admin.exclusiveOffers.noProducts', 'No products found')}</Text></View>}
            />
          )}
          <View style={styles.modalFooter}>
            <Button title={t('common.done', 'Done')} onPress={() => setShowProductModal(false)} style={styles.modalDoneBtn} />
          </View>
        </SafeAreaView>
      </Modal>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  label: { fontSize: FONT_SIZES.sm, fontWeight: FONT_WEIGHTS.medium, color: COLORS.text, marginBottom: SPACING.xs },
  fieldRow: { marginBottom: SPACING.md },
  dropdown: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: COLORS.surface, borderWidth: 1, borderColor: COLORS.border, borderRadius: BORDER_RADIUS.md, paddingHorizontal: SPACING.md, paddingVertical: SPACING.md },
  ...
  productSelectBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: COLORS.surface, borderWidth: 1, borderColor: COLORS.border, borderRadius: BORDER_RADIUS.md, paddingHorizontal: SPACING.md, paddingVertical: SPACING.md },
  ...
  modalBox: { backgroundColor: COLORS.background, borderRadius: BORDER_RADIUS.lg, width: '85%', maxWidth: 340, padding: SPACING.lg },
  modalTitle: { fontSize: FONT_SIZES.lg, fontWeight: FONT_WEIGHTS.bold, color: COLORS.text, marginBottom: SPACING.md },
  modalOption: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: SPACING.md, borderRadius: BORDER_RADIUS.md, marginBottom: SPACING.sm, backgroundColor: COLORS.surface },
  ...
  productRow: { flexDirection: 'row', alignItems: 'center', padding: SPACING.md, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: COLORS.border, gap: SPACING.sm },
...
export default AdminEditExclusiveOfferScreen;

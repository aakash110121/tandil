...
const OFFER_PLACEHOLDER = 'https://images.unsplash.com/photo-1461354464878-ad92f492a5a0?w=400';
...
const OffersScreen: React.FC = () => {
  ...
  const renderOfferCard = ({ item }: { item: PublicExclusiveOffer }) => {
    const uri = getOfferImageUrl(item) || OFFER_PLACEHOLDER;
    return (
      <TouchableOpacity
        style={styles.card}
      ...
      >
        ...
        <View style={styles.cardContent}>
          <Text style={styles.cardTitle} numberOfLines={1}>{offerTitle(item)}</Text>
          <Text style={styles.cardSubtitle} numberOfLines={2}>{offerSubtitle(item)}</Text>
        </View>
      </TouchableOpacity>
    );
  };
  ...
  return (
    <View style={styles.container}>
      <Header
        title={t('home.exclusiveOffers', 'Exclusive Offers')}
        showBack

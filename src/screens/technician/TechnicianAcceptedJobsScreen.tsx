...
function mapTaskToJob(task: TechnicianTodayTask) {
  const status = (task.status ?? 'accepted').toLowerCase().replace(/\s+/g, '_');
  const durationStr =
...
const TechnicianAcceptedJobsScreen: React.FC = () => {
  ...
  const loadJobs = useCallback(
    async (page: number = 1, append: boolean = false, periodOverride?: TechnicianJobsPeriod) => {
      ...
      try {
        const data = await getTechnicianAcceptedJobs(p, PER_PAGE, page);
        setResult(prev => {
        ...
        <View style={styles.jobHeader}>
          ...
          <View
            style={[
              ...
              {
                backgroundColor:
                  item.status === 'in_progress' ? COLORS.primary + '20' : COLORS.info + '20',
              },
            ]}
          >
            <Text
...
export default TechnicianAcceptedJobsScreen;
